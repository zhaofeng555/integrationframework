...
//			String key=clazz.getName().concat("_ids");
	...
	public String setBean(Object obj) {
		String c="";
		ShardedJedis jedis = shardedJedisPool.getResource();
		try {
			...
			jedis.zadd(idsKey, s, String.valueOf(id));
			
			//set bean
			String key =getBeanKey(clazz, id);
			String value = JSONObject.toJSONString(obj);
			c=jedis.set(key, value);
...
//			//shardedJedisPool.returnBrokenResource(jedis);
	 ...
	 */
	public String setBeans(List objList) {
		String c="";
		ShardedJedis jedis = shardedJedisPool.getResource();
		try {
	...
	public <T> T getBeanById(Class<T> clazz, Serializable id) {
		...
		T tmp = JSONObject.parseObject(value, clazz);
		return tmp;
	}
	...
	/* (non-Javadoc)
	 * @see hjg.cache.RedisCacheDao#getBeanKey(java.lang.Class, java.io.Serializable)
	 */
